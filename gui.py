    ...
    def _convert_books(self, rows):
	    ...
	    for book_id in ids:
		   ...
		   job = ThreadedJob('ConvertToDJVU',
                   ...
                   self.gui.job_manager.run_threaded_job(job) #too bad console utils and filetype plugins can't start a jobmanager..fork_job is a wretch
        else: #!gui_library
	    #looking at a device's flash contents or some other non-library store, filepaths here are not to be tracked in the db
